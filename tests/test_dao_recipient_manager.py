...
class TestRecipientManager:
    @pytest.mark.parametrize(
        "is_none,recipients,id,res_none",
        [
            (False, [1, 2, 3], 1, False),
            (False, [1, 2, 3], 4, True),
            (False, [], 1, True),
    ...
    def test_retrieve_by_id(self, is_none, recipients, id, res_none):
        if is_none:
            ...
            _message = Message(
                recipients=[Recipient(id_recipient=i) for i in recipients]
            )
            if res_none:
                assert RecipientManager.retrieve_recipient_by_id(_message, id) == None
            else:
                assert (
                    RecipientManager.retrieve_recipient_by_id(_message, id).id_recipient
                    == id
                )
    ...
    @pytest.mark.parametrize(
        "is_none,recipients,id,old_value,result",
        [
    ...
    @pytest.mark.parametrize(
        "is_none,recipients,id,is_arrived,result",
        [
            (False, [1, 2, 3], 1, True, True),
            (False, [1, 2, 3], 1, False, False),
    ...
    @pytest.mark.parametrize(
        "is_none,recipients,id,has_opened,result",
        [
            (False, [1, 2, 3], 1, True, True),
            (False, [1, 2, 3], 1, False, False),
            (False, [1, 2, 3], 4, True, False),
    ...
    def test_delete_read_message(self, is_none, recipients, id, has_opened, result):
        if is_none:
            ...
            rcps = [
                Recipient(id_recipient=i, has_opened=has_opened) for i in recipients
            ]
            _message = Message(recipients=rcps)
            assert RecipientManager.delete_read_message(_message, id) == result
            if result:
                assert (
                    next(r for r in rcps if r.id_recipient == id).read_deleted == True
                )

    @pytest.mark.parametrize(
        "replying,recipients,check",
        [
            (False, [1, 2, 3], [1, 2, 3]),
            (False, [1, 1, 2, 3], [1, 2, 3]),
            (False, [], []),
