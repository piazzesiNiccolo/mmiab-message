...
class TestMessageManager:
    ...
    def test_get_sent_messages(self, sent_list, day_dt, month_dt, nres):
        _messages = MessageManager.get_sent_messages(
            1, day_dt=day_dt, month_dt=month_dt
        )
    ...
    def test_get_received_messages(self, received_list, day_dt, month_dt, cf, nres):
        ...
        with mock.patch(
            "mib.dao.message_manager.MessageManager.get_user_content_filter"
        ) as m:
            m.return_value = 200, cf
            _messages, _dict = MessageManager.get_received_messages(
                1, day_dt=day_dt, month_dt=month_dt
            )
            assert all(
                map(
                    lambda m: 1 in [r.id_recipient for r in m.recipients]
                    and m.is_sent == True
                    and m.is_arrived == True,
                    _messages,
                )
            )
    ...
    @pytest.mark.parametrize(
        "is_sent,is_arrived,id_user,result",
        [
            (True, True, 1, True),
            (True, True, 2, True),
            (True, True, 3, False),
    ...
    @pytest.mark.parametrize(
        "code,id_list,deep_list,diff_elem,called",
        [
            (200, [], [], 0, False),
            (404, [1], [[2]], 0, True),
            (200, [], [[], [], []], 0, False),
    ...
    def test_retrieve_users_info(self, code, id_list, deep_list, diff_elem, called):
        ...
        with mock.patch("requests.get", new=mock_get_fun):
            dict = MessageManager.retrieve_users_info(
                id_list=id_list, deep_list=deep_list
            )
