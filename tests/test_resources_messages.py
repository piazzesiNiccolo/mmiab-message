...
class TestMessages:
    ...
    def test_draft(self, test_client, data, code):
        ...
        if code == 201:
            ...
            assert [r.id_recipient for r in msg.recipients] == data.get(
                "recipients", []
            )
            if "image" in data:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                Message(
                    is_arrived=True,
                    recipients=[Recipient(id_recipient=1, has_opened=False)],
                ),
                400,
            ),
            (
                Message(
                    is_arrived=True,
                    recipients=[Recipient(id_recipient=1, has_opened=True)],
                ),
                200,
            ),
        ],
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                Message(
                    id_sender=1, is_sent=False, recipients=[Recipient(id_recipient=2)]
                ),
                400,
            ),
            (
    ...
    def test_withdraw_message(self, test_client, message, lp, code):
        if message is not None:
            ...
            db.session.commit()
        with mock.patch(
            "mib.dao.message_manager.MessageManager.get_user_lottery_points"
        ) as m:
            m.return_value = lp
        ...
        [
            ...
            (
                Message(
                    ...
                    is_sent=True,
                    is_arrived=True,
                    recipients=[Recipient(id_recipient=3)],
                ),
                403,
            ),
            (
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                Message(
                    id_sender=2, is_arrived=True, recipients=[Recipient(id_recipient=3)]
                ),
                401,
            ),
            (
                Message(
                    id_sender=1, is_arrived=True, recipients=[Recipient(id_recipient=3)]
                ),
                200,
            ),
            (
                Message(
                    id_sender=2, is_arrived=True, recipients=[Recipient(id_recipient=1)]
                ),
                200,
            ),
            (
    ...
    def test_message_list_draft(self, test_client, draft_list):
        with mock.patch(
            "mib.dao.message_manager.MessageManager.retrieve_users_info"
        ) as m:
    ...
    @pytest.mark.parametrize(
        "uri,year,month,sent_res,rcv_res",
        [
    ...
    def test_message_list_monthly(
        self, test_client, received_list, sent_list, uri, year, month, sent_res, rcv_res
    ):
