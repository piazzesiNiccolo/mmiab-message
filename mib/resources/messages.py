...
def delete_read_message(id_message, id_user):
    ...
    analysis = [
        ...
        (
            lambda: not RM.can_delete_read(msg, id_user),
            "User not allowed to delete the message",
            403,
        ),
        (
            lambda: not RM.delete_read_message(msg, id_user),
            "You cannot delete an unread message",
            400,
        ),
    ]
...
def delete_draft(id_message, id_sender):
    ...
    analysis = [
        (lambda: msg is None, "Message not found", 404),
        (
            lambda: msg.id_sender != id_sender,
            "User not allowed to send the message",
            403,
        ),
...
def send_message(id_message, id_user):
    ...
    analysis = [
        ...
        (
            lambda: len(msg.recipients) == 0,
            "You must pick a list of recipients first",
            400,
        ),
    ]
    for fail, message, code in analysis:
...
def withdraw_message(id_message, id_user):
    ...
    analysis = [
        (lambda: msg is None, "Message not found", 404),
        (
            lambda: msg.id_sender != id_user,
            "User not allowed to withdraw the message",
            403,
        ),
        (lambda: msg.is_sent == False, "You cannot withdraw a draft", 400),
        (
            lambda: msg.is_arrived == True,
            "You cannot withdraw a delivered message",
            400,
        ),
        (
            lambda: MM.get_user_lottery_points(id_user) <= 0,
            "You don't have enough lottery points",
            400,
        ),
    ]
    for fail, message, code in analysis:
...
def can_reply(id_message, id_user):
    ...
    analysis = [
        (lambda: msg is None, "Message not found", 404),
        (
            lambda: not MM.user_can_reply(id_user, msg),
            "User not allowed to reply to the message",
            403,
        ),
    ]
    for fail, message, code in analysis:
...
def forward_message(id_message, id_user):
    ...
    analysis = [
        (lambda: msg is None, "Message not found", 404),
        (
            lambda: not MM.user_can_forward(id_user, msg),
            "User not allowed to forward the message",
            403,
        ),
    ]
    for fail, message, code in analysis:
...
def get_replying_info(id_message, id_user):
    ...
    if message == None:
        response_object = {"status": "failed", "message": "Message not found"}
        return jsonify(response_object), 404
    ...
    users_info = MM.retrieve_users_info(id_list=[message.id_sender])
    response_object = {
        "status": "success",
